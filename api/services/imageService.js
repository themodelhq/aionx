  ...
  async generate({ id, prompt, style = 'realistic', aspectRatio = '1:1', quality = 'standard', numImages = 1 }) {
    ...
    for (let i = 0; i < numImages; i++) {
      try {
        ...
        const imageBuffer = await this.generateWithGemini(prompt, style, aspectRatio, geminiKey);        writeFileSync(filepath, imageBuffer);

        const resultUrl = `/uploads/generated/images/${filename}`;
        results.push({ url: resultUrl, thumbnail: resultUrl, index: i });
        console.log(`✓ Image ${i + 1}/${numImages} generated via Gemini Imagen`);
      } catch (error) {
